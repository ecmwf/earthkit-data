...
The pivotal question when generating the Xarray dataset is how to form the dimensions. The :py:meth:`~earthkit.data.indexing.xarray.XarrayMixIn.to_xarray` method has a number of options to control the dimensions. Please see more details in the :ref:`dimensions <xr_dim>` section.


Profiles
+++++++++

:py:meth:`~earthkit.data.indexing.xarray.XarrayMixIn.to_xarray` has a large number of keyword arguments to control how the Xarray dataset is generated. To simplify the usage we can define :ref:`profiles <xr_profile>` providing custom defaults for most of the keyword arguments. For more details see :ref:`xr_profile`.


Examples
+++++++++

The following notebooks give details about how :py:meth:`~earthkit.data.indexing.xarray.XarrayMixIn.to_xarray` can be used:

- :ref:`Xarray engine examples <xr_engine_examples>`
...
- :ref:`/tutorials/xr_engine/xarray_engine_to_grib.ipynb`
...
To convert GRIB data to NetCDF first we need to convert GRIB to Xarray with :py:meth:`~earthkit.data.indexing.xarray.XarrayMixIn.to_xarray` then generate NetCDF from it with :py:func:`xarray.Dataset.to_netcdf`.

Using to_netcdf() directly
++++++++++++++++++++++++++++

The earthkit Xarray engine attaches some special attributes (``_earthkit``) to each DataArray in the generated Xarray dataset. These attributes are encoded as JSON strings, so they can be written to NetCDF without any issues. You can simply call :py:func:`xarray.Dataset.to_netcdf` directly on the dataset.
...
If you prefer not to include the earthkit attributes in the NetCDF file at all, you can

* use the ``add_earthkit_attrs=False`` option in :py:meth:`~earthkit.data.indexing.xarray.XarrayMixIn.to_xarray`.
* use the ``remove_earthkit_attrs=True`` option in the call ``to_netcdf()`` on the ``earthkit`` accessor.

Note that without these attributes, converting the Xarray dataset back to GRIB will not be possible.
...
We can also use the :func:`to_target` method to convert GRIB directly to NetCDF. This method will generate an Xarray dataset and write it to a NetCDF file in one step.
...
- :ref:`/tutorials/grib/grib_to_netcdf.ipynb`
