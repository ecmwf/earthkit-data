...
- Implemented mono variable (single dataarray containing all the parameters from a GRIB fieldlist) in Xarray engine (:pr:`734`). See the notebook examples:
  ...
  -  :ref:`/tutorials/xr_engine/xarray_engine_mono_variable.ipynb`
  -  :ref:`/tutorials/xr_engine/xarray_engine_mono_variable_remapping.ipynb`

- Allowed specifying metadata defaults for :py:meth:`~earthkit.data.indexing.xarray.XarrayMixIn.to_xarray` (:pr:`738`) via the ``fill_metadata`` option. See the notebook example :ref:`/tutorials/xr_engine/xarray_engine_mono_variable.ipynb`
...
- Added GPU support for the Xarray engine (:pr:`745`). See the notebook example :ref:`/tutorials/xr_engine/xarray_cupy.ipynb`
