...
The :func:`to_grib` method on the ``earthkit`` Xarray accessor is deprecated. Use :func:`to_target` instead. The method is still available for backward compatibility but will be removed in a future release. See :ref:`/tutorials/xr_engine/xarray_engine_to_grib.ipynb` notebook for details on how to use the new API.
