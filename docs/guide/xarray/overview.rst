...
Xarray created with the earthkit engine can be converted back to GRIB. Please note this is an **experimental feature**. See details in the :ref:`xr_to_grib` section.


Converting GRIB to NetCDF
+++++++++++++++++++++++++

To convert GRIB data to NetCDF first we need to convert GRIB to Xarray with :py:meth:`~data.readers.grib.index.GribFieldList.to_xarray` then generate NetCDF from it with :py:meth:`xarray.Dataset.to_netcdf`. See details in the :ref:`xr_grib_to_netcdf` section.
