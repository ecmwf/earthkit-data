...
.. py:function:: from_source("url", url, unpack=True, parts=None, stream=False, read_all=False)
  ...
  :param bool read_all: when it is ``True`` all the data is read straight to memory from a stream. Used when ``stream=True``.
...
.. py:function:: from_source("stream", stream, read_all=False)
  ...
  :param bool read_all: when it is ``True`` all the data is read into memory from a stream. Used when ``stream=True``.
...
.. py:function:: from_source("fdb", *args, stream=True, read_all=False, **kwargs)
  ...
  :param bool stream: when it is ``True`` the data is read as a stream. Otherwise it is retrieved into a file and stored in the :ref:`cache <caching>`. Stream-based access only works for :ref:`grib` and CoverageJson data.
  :param bool read_all: when it is ``True`` all the data is read into memory from a stream. Used when ``stream=True``.
  :param dict **kwargs: other keyword arguments specifying the request
...
.. py:function:: from_source("polytope", collection, *args, address=None, user_email=None, user_key=None, stream=True, read_all=False, **kwargs)
  ...
  :param bool stream: when it is ``True`` the data is read as a stream. Otherwise it is retrieved into a file and stored in the :ref:`cache <caching>`. Stream-based access only works for :ref:`grib` data.
  :param bool read_all: when it is ``True`` all the data is read into memory from a stream. Used when ``stream=True``.
