...
def test_netcdf_single_index(mode, index, expected_meta):
    ...
    r = f[index]
    assert r.get(["param", "level"]) == expected_meta
    v = r.values
    assert v.shape == (84,)
    # eps = 0.001
...
def test_netcdf_slice_single_file(mode, indexes, expected_meta):
    ...
    assert len(f) == 18
    assert f.metadata("param") == ["t"] * 6 + ["u"] * 6 + ["v"] * 6
...
def test_netcdf_array_indexing(mode, indexes1, indexes2):
    ...
    r = f[indexes1]
    assert len(r) == 4
    assert r.get("param") == ["t", "v", "t", "u"]

    r1 = r[indexes2]
    assert len(r1) == 2
    assert r1.get("param") == ["v", "u"]
...
def test_netcdf_fieldlist_iterator(mode):
    ...
    sn = g.get("param")
    assert len(sn) == 18
    iter_sn = [f.get("param") for f in g]
    assert iter_sn == sn
    # repeated iteration
    iter_sn = [f.get("param") for f in g]
    assert iter_sn == sn
...
def test_netcdf_fieldlist_reverse_iterator(mode):
    ...
    gr = reversed(g)
    iter_sn = [f.get("param") for f in gr]
