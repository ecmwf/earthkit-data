...
def test_xr_engine_builder_fieldlist(representation):
    ...
    assert r_p.ds.get("metadata.time", astype=int) == [
        0,
        0,
        0,
        0,
    ...
    r0 = r.sel({"metadata.param": "t", "metadata.level": 500})
    assert not isinstance(r0.ds, XArrayInputFieldList)
    assert len(r0) == 8
    assert r0.ds.get("metadata.time", astype=int) == [0, 0, 1200, 1200, 0, 0, 1200, 1200]
    r0_uw = r0.unwrap()
    assert not isinstance(r0_uw, XArrayInputFieldList)
    assert r0_uw.get("metadata.time", astype=int) == [0, 0, 1200, 1200, 0, 0, 1200, 1200]
    r0_p = _pickle(r0, representation)
    assert r0_p is not r0
    assert r0_p.ds is not r0.ds
    assert r0_p.ds.get("metadata.time", astype=int) == [0, 0, 1200, 1200, 0, 0, 1200, 1200]

    r1 = r0.order_by("metadata.time")
    assert not isinstance(r1.ds, XArrayInputFieldList)
    assert r1.ds.get("metadata.time", astype=int) == [0, 0, 0, 0, 1200, 1200, 1200, 1200]
    r1_uw = r1.unwrap()
    assert not isinstance(r1_uw, XArrayInputFieldList)
    assert r1_uw.get("metadata.time", astype=int) == [0, 0, 0, 0, 1200, 1200, 1200, 1200]
    r1_p = _pickle(r1, representation)
    assert r1_p is not r1
    assert r1_p.ds is not r1.ds
    assert r1_p.ds.get("metadata.time", astype=int) == [0, 0, 0, 0, 1200, 1200, 1200, 1200]

    r2 = r1.order_by("metadata.step")
    assert not isinstance(r2.ds, XArrayInputFieldList)
    assert r2.ds.get("metadata.time", astype=int) == [0, 0, 1200, 1200, 0, 0, 1200, 1200]
    assert r2.ds.get("time.step", astype=timedelta_to_hours) == [0, 0, 0, 0, 6, 6, 6, 6]
    assert r2.ds.get("metadata.step", astype=int) == [0, 0, 0, 0, 6, 6, 6, 6]
    r2_uw = r2.unwrap()
    assert not isinstance(r2_uw, XArrayInputFieldList)
    assert r2_uw.get("metadata.time", astype=int) == [0, 0, 1200, 1200, 0, 0, 1200, 1200]
    assert r2_uw.get("time.step", astype=timedelta_to_hours) == [0, 0, 0, 0, 6, 6, 6, 6]
    assert r2_uw.get("metadata.step", astype=int) == [0, 0, 0, 0, 6, 6, 6, 6]
    r2_p = _pickle(r2, representation)
    assert r2_p is not r2
    assert r2_p.ds is not r2.ds
    assert r2_p.ds.get("metadata.time", astype=int) == [0, 0, 1200, 1200, 0, 0, 1200, 1200]
    assert r2_p.ds.get("time.step", astype=timedelta_to_hours) == [0, 0, 0, 0, 6, 6, 6, 6]
    assert r2_p.ds.get("metadata.step", astype=int) == [0, 0, 0, 0, 6, 6, 6, 6]
