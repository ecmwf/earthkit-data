...
from earthkit.data.encoders.bufr import BufrEncoder
...
def test_target_file_grib_core_stream(kwargs, direct_call):
    ds = from_source("file", earthkit_examples_file("test.grib"), stream=True).to_fieldlist()
    ds_ref = from_source("file", earthkit_examples_file("test.grib")).to_fieldlist()
...
@pytest.mark.parametrize("direct_call", [True, False])
def test_target_file_bufr_core(kwargs, direct_call):
    ds = from_source("file", earthkit_examples_file("temp_10.bufr")).to_featurelist()
    md_ref = [2] * 10
    assert ds.get("dataCategory") == md_ref
    ...
    with temp_file() as path:
        ...
        ds1 = from_source("file", path).to_featurelist()
        assert len(ds) == len(ds1)
        assert ds1.get("dataCategory") == md_ref
...
@pytest.mark.parametrize("direct_call", [True, False])
def test_target_file_bufr_slice(kwargs, direct_call):
    ds_in = from_source("file", earthkit_examples_file("temp_10.bufr")).to_featurelist()
    ds = ds_in[1:4]
    print("ds", ds)
    md_ref = ["01400", "01415", "01001"]
    assert ds.get("ident") == md_ref
...
def test_target_file_xarray_to_netcdf():
    ds_in = from_source("file", earthkit_examples_file("test.grib")).to_fieldlist()
    # vals_ref = ds.values[:, :4]
