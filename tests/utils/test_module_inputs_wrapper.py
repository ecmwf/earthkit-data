...
from . import dummy_module  # noqa: F402
from .dummy_module import XR_TYPES  # noqa: E402
...
WRAPPED_NP_MEAN = module_inputs_wrapper.transform_function_inputs(
    ...
    convert_types=(Data, Reader),  # Only convert Earthkit.data.Reader (np.mean can handle xarray and pandas)
)
...
def test_transform_function_inputs_gribdata_to_xarray():
    # Check EK GribData object
    EK_GRIB_DATA = from_source("file", earthkit_test_data_file("test_single.grib"))
    ek_reader_result = WRAPPED_XR_ONES_LIKE(EK_GRIB_DATA)
    # Will return a DataSet because that is first value in kwarg_types
    assert isinstance(ek_reader_result, xr.Dataset)
    assert ek_reader_result.equals(xr.ones_like(EK_GRIB_DATA.to_xarray()))


def test_transform_function_inputs_gribdata_to_xarray_typesetting():
    # Check EK GribData object
    EK_GRIB_DATA = from_source("file", earthkit_test_data_file("test_single.grib"))
    ek_reader_result = WRAPPED_XR_ONES_LIKE_TYPE_SETTING(EK_GRIB_DATA)
    # Will return a dataarray because that is first value in type-set Union
    assert isinstance(ek_reader_result, xr.DataArray)
    assert ek_reader_result.equals(xr.ones_like(EK_GRIB_DATA.to_xarray()["2t"]))


def test_transform_module_inputs_gribdata_to_xarray():
    # Check EK GribData object
    EK_GRIB_DATA = from_source("file", earthkit_test_data_file("test_single.grib"))
    ek_reader_result = WRAPPED_DUMMY_MODULE.xarray_ones_like(EK_GRIB_DATA)
...
def test_transform_function_inputs_wrapper_to_xarray():
    ...
    assert ek_wrapper_result.equals(xr.ones_like(EK_XARRAY_WRAPPER.to_xarray()))
...
def test_transform_module_inputs_wrapper_to_xarray():
    ...
    # Will return a dataarray because that is first value in type-set Union
    assert isinstance(ek_wrapper_result, xr.DataArray)
    assert ek_wrapper_result.equals(xr.ones_like(EK_XARRAY_WRAPPER.to_xarray()["test"]))
    # EK NumpyWrapper object
...
def test_transform_function_inputs_gribdata_to_numpy():
    # Test with Earthkit.data GribData object
    EK_GRIB_DATA = from_source("file", earthkit_test_data_file("test_single.grib"))
    assert WRAPPED_NP_MEAN(EK_GRIB_DATA) == np.mean(EK_GRIB_DATA.to_numpy())
    assert isinstance(WRAPPED_NP_MEAN(EK_GRIB_DATA), np.float64)


def test_transform_function_inputs_gribdata_to_numpy_typesetting():
    # Test with Earthkit.data GribData object
    EK_GRIB_DATA = from_source("file", earthkit_test_data_file("test_single.grib"))
    result = WRAPPED_NP_MEAN_TYPE_SETTING(EK_GRIB_DATA)
    assert result == np.mean(EK_GRIB_DATA.to_numpy())
    assert isinstance(result, np.float64)


def test_transform_module_inputs_gribdata_to_numpy():
    # Test with Earthkit.data GribData object
    EK_GRIB_DATA = from_source("file", earthkit_test_data_file("test_single.grib"))
    result = WRAPPED_DUMMY_MODULE.numpy_mean(EK_GRIB_DATA)
    assert result == np.mean(EK_GRIB_DATA.to_numpy())
    assert isinstance(result, np.float64)
