...
def test_formats():
    ...
    assert FormatTransformer(None, type=EnumListType(enum), format="%4s").transform(("a", "b")) == [
        "   a",
        "   b",
    ]
    ...
    assert FormatTransformer(None, type=FloatListType, format="%.1f").transform((3.14, 2.72)) == [
        "3.1",
        "2.7",
    ]
