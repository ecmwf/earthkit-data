...
def test_s3_ewc_public_single_parts(parts, expected_meta):
    parts_kwargs = dict(start=parts[0], range=parts[1])

    r = {
        "endpoint": "object-store.os-api.cci1.ecmwf.int",
        "bucket": "earthkit-test-data-public",
        "objects": [{"object": "test6.grib", **parts_kwargs}],
    }
...
def test_s3_stream_single_parts(parts, expected_meta):
    parts_kwargs = dict(start=parts[0], range=parts[1])
