...
DOT_EMOHAWK = os.path.expanduser("~/.emohawk")
...
SETTINGS_AND_HELP = {
    "cache-directory": _(
        os.path.join(tempfile.gettempdir(), "emohawk-%s" % (getpass.getuser(),)),
    ...
    "datasets-directories": _(
        [os.path.join(DOT_EMOHAWK, "datasets")],
...
save = False
settings_yaml = os.path.expanduser(os.path.join(DOT_EMOHAWK, "settings.yaml"))

try:
    if not os.path.exists(DOT_EMOHAWK):
        os.mkdir(DOT_EMOHAWK, 0o700)
