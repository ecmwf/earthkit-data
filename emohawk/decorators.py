...
from emohawk.utils import load_json_or_yaml
...
class Decorator:
    ...
    def __call__(self, func):
        from emohawk.arguments import InputManager
        ...
        def unwrap(f):
            if not hasattr(f, "_emohawk_decorators"):
                return f
            return unwrap(f.__wrapped__)

        unwrapped = unwrap(func)

        if hasattr(func, "_emohawk_decorators"):
            decorators = decorators + func._emohawk_decorators
        ...
        newfunc._emohawk_decorators = decorators
