...
from .indexed import IndexFeatureListBase


class SimpleFeatureListBase(IndexFeatureListBase):
    @property
    @abstractmethod
    def _features(self):
        pass
    ...
    def __getstate__(self) -> dict:
        ret = {}
        # print("SimpleFeatureList Getstate")
        ret["_features"] = self._features
        return ret
    ...
    def merge(cls, sources):
        for s in sources:
            if not isinstance(s, SimpleFeatureListBase):
                raise ValueError("SimpleFeatureList can only be merged to another SimpleFeatureLists")

        if not all(isinstance(_, SimpleFeatureListBase) for _ in sources):
            raise ValueError("SimpleFeatureList can only be merged to another SimpleFeatureLists")
...
class SimpleFeatureList(SimpleFeatureListBase):
    def __init__(self, fields=None):
        r"""Initialize a FeatureList object."""
        if isinstance(fields, Field):
            fields = [fields]

        self.__features = fields if fields is not None else []
