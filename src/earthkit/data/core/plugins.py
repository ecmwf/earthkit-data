...
"""Plugins rely on the python plugins_ system using ``entry_points``.
...
def _load_plugins(kind):
    plugins = {}
    for e in entrypoints.get_group_all(f"earthkit.data.{kind}s"):
...
def directories(owner: bool = False) -> list:
    ...
    result.append(("earthkit.data", os.path.dirname(earthkit.data.__file__)))
